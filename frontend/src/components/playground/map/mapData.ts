...
// y6-17:  [경영(0-11) | 개발팀(12-35) | 플랫폼팀(36-47) | Meeting Room(48-59)]
...
export const TEAM_EN: Record<string, string> = Object.fromEntries(
  Object.entries(TEAM_CONFIG).map(([k, v]) => [k, v.labelEn])
);

export const ROOM_FLOORS: Record<string, { type: string; base: string; accent: string }> = Object.fromEntries(
  Object.entries(TEAM_CONFIG).map(([k, v]) => [k, v.floor])
);
...
export const ROOMS: RoomDef[] = [
  ...
  // Meeting Room
  { team: 'Meeting',    x: 48, y: 6,  w: 12, h: 12, color: '#0c0c14', wall: '#64748b40', label: '#94a3b8' },
  // Lower floor (y22-33)
...
export const POI_LIST: POIDef[] = [
  ...
  { name: 'whiteboard',   col: 24, row: 7,  action: '화이트보드 정리 중', type: 'indoor' },
  ...
  { name: 'smoking',      col: 8,  row: 38, action: '흡연 중', type: 'outdoor' },
  { name: 'smoking_b',    col: 12, row: 38, action: '바람 쐬는 중', type: 'outdoor' },
