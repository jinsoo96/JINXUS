...
class BackgroundWorker:
    ...
    async def _execute_task(self, task: BackgroundTask, worker_id: int):
        ...
        logger.info(f"[Worker-{worker_id}] 작업 시작: {task.task_id[:8]}")
        ...
        try:
            ...
            # JINXUS 실행 (progress_callback 전달)
            ...
            logger.info(f"[Worker-{worker_id}] 작업 완료: {task.task_id[:8]} ({duration:.1f}초), 첨부: {len(task.attachments)}개")
