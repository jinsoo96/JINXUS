    ...
    async def run(self, instruction: str, context: list = None) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[JXOps] 장기 메모리 검색 실패, 컨텍스트 없이 진행: {e}")
...
사용 가능한 action:
- list_user_repos: 사용자의 레포지토리 목록 조회 (username 필요, limit 선택)
- search_repos: 레포지토리 검색 (query 필요, 예: "user:jinsoo96" 또는 "language:python")
- get_repo: 레포 정보 조회 (repo 필요, 예: "owner/repo")
...
JSON 형식으로만 응답해:
```json
{{"action": "list_user_repos", "username": "jinsoo96"}}
```

```json
{{"action": "get_repo", "repo": "owner/repo"}}
```
...
이 작업에서 배운 핵심 포인트를 1-2문장으로 정리해줘."""
    ...
    async def _memory_write(
        ...
        try:
            ...
            logger.warning(f"[JXOps] 장기 메모리 저장 실패 (결과 반환은 정상): {e}")
