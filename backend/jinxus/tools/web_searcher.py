...
class WebSearcher(JinxTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "mode": {
                "type": "string",
                "description": "검색 모드: 'search' (일반) 또는 'news' (뉴스)",
                "enum": ["search", "news"],
                "default": "search"
            },
            "max_results": {
            ...
            "auto_expand": {
                "type": "boolean",
                "description": "쿼리 자동 변형 여부",
                "default": True
            },
            "days": {
                "type": "integer",
                "description": "뉴스 검색 시 최근 N일 (기본 7, mode=news일 때만 적용)",
                "default": 7
            }
        },
        "required": ["query"]
    ...
    async def run(self, input_data: dict) -> ToolResult:
        ...
        # 뉴스 검색 모드
        if mode == "news":
            return await self.search_news(query, input_data.get("days", 7))
