...
class TelegramBot:
    ...
    async def _cmd_auto(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/auto 명령 - 자율 멀티스텝 작업 실행

        복잡한 작업을 여러 단계로 분해하여 자율적으로 실행.
        예: /auto 이 프로젝트 분석하고 코드 품질 개선해줘
        """
        if not update.message:
            return
        ...
        if not task_description:
            await update.message.reply_text(
                "자율 작업 내용을 입력해주세요.\n"
                "예: /auto 프로젝트 전체 분석하고 개선사항 적용\n\n"
                "복잡한 작업을 여러 단계로 분해하여 자율 실행합니다.\n"
                "최대 10단계, 4시간 타임아웃."
            )
            return
        ...
        try:
            ...
            task_id = await worker.submit(
                ...
                session_id=f"telegram_auto_{chat_id}",
            ...
            await update.message.reply_text(
                ...
                f"모드: 자율 멀티스텝 (최대 10단계)\n\n"
                f"단계별 진행 상황을 알려드립니다.\n"
                f"취소: /cancel {task_id[:8]}"
            )

        except Exception as e:
            logger.error(f"Autonomous task submission error: {e}")
            await update.message.reply_text(f"자율 작업 등록 오류: {str(e)[:200]}")
